...
create table if not exists TRAINEE (date_of_birth timestamp(6) not null, id bigint not null primary key DEFAULT nextval('trainee_sequence'), user_id bigint not null unique, address varchar(255) not null);
create table if not exists TRAINER (id bigint not null primary key DEFAULT nextval('trainer_sequence'), specialization_id bigint not null, user_id bigint not null unique);
create table if not exists TRAINING (date timestamp(6) not null, duration bigint not null, id bigint not null primary key DEFAULT nextval('training_sequence'), trainee_id bigint, trainer_id bigint, training_type_id bigint not null, training_name varchar(255) not null);
create table if not exists TRAINING_TYPE (id bigint not null primary key DEFAULT nextval('training_type_sequence'), training_type varchar(255) check (training_type in ('STRENGTH_TRAINING','AEROBIC','BODYBUILDING','FLEXIBILITY_TRAINING','WEIGHTLIFTING')));
create table if not exists USERS (is_active boolean, id bigint not null primary key DEFAULT nextval('user_sequence'), first_name varchar(255) not null, last_name varchar(255) not null, password varchar(255) not null, username varchar(255) not null);
